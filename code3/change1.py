...
def distance1(point,cluster,k,n):
    dis = []
    distance = 0
    for i in range(0,k):
        for s in range(0,n):
    ...
    return minimum
def collecta(fileName,cluster,k,n):
        lines1 = sc.textFile (fileName)
        words1 = lines1.flatMap (lambda line: line.split('\n'))
        pairs1 = words1.map(lambda word:(distance(mapperagain(n,word),cluster,k,n),distance1(mapperagain(n,word),cluster,k,n)))
        counts1 = pairs1.reduceByKey(lambda a,b: [a]+[b])
        results1 = counts1.collect()
        printList(results1)
